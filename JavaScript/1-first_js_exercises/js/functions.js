...
function dniLetter( dni ) {
	...
	var result
	var inputErrorMessage = 'Invalid parameter';
	var dniString = String(dni);

	if ((dniString.length !== 8) || (dniString.charAt(0) === (0 && '-')) || (typeof(dni) !== 'number')) {
...
function findLongestWord( array_words ) {
	...
	for (i in array_words) {
		words_lengths.push(array_words[i].length);
	}

	return Math.max(...words_lengths);
}
...
array_lengths.push( findLongestWord( ["rope", "batarang", "darts"] ) );
array_lengths.push( findLongestWord( ["rope", "batarang", "darts"] ) );
...
console.log( calculateAverage( array_lengths ) && "The average of longest lengths is: " + calculateAverage( array_lengths ))
...
function charFreq( string ) {
	freqCounter = {};
	for (i in string) {
		if (!(Object.keys(freqCounter)).includes(string.charAt(i))) {
...
function arrangeElements( array ) {
	var letters = [], numbers = [];
	for (i in array) {
		(isFinite(array[i])) ? (numbers.push(array[i])) : (letters.push(array[i]));
	}
...
function beautifyLetters( array ) {
	...
	for (i in array) {
		if (typeof(array[i]) === 'string') {
				(vowels.includes(array[i])) ? (array[i] = array[i].toUpperCase()) : (array[i] = array[i].toLowerCase());
		}
	}
	return array;
}
	...
	for (i in array) {
		if (isFinite(array[i])) {
			while ((array[i].toString()).length > 1) {
				var sum = 0, num = array[i].toString();
				num = num.split('');
				for (n in num) {
					sum = sum + parseInt(num[n]);
				}
				array[i] = sum;
			}
		}
...
function sortArray( array ) {
	var letters = [], numbers = [];
	for (i in array) {
		(Number.isFinite(array[i])) ? (numbers.push(array[i])) : (letters.push(array[i]));
		}
	return ((numbers.sort()).concat(letters.sort(function(a, b) {return ((a.toLowerCase()).charCodeAt(0)-(b.toLowerCase()).charCodeAt(0))})));
}
